...
const Articles = {
  // GUARDAR NOTA RASPADA (borrador). Si ya existe, solo refresca la fecha;
  // NUNCA borra el contenido reescrito ni cambia el estado de una nota ya publicada.
  upsert(article) {
    const db = getDB();
    const stmt = db.prepare(`
      INSERT INTO articles (
        id, title, description, url, source, source_weight,
        alcaldia, geo_confidence, category, pub_date, scraped_at, status
      ) VALUES (
        @id, @title, @description, @url, @source, @source_weight,
        @alcaldia, @geo_confidence, @category, @pub_date, @scraped_at, @status
      )
    ...
    return stmt.run({
      ...
      source_weight: article.sourceWeight || article.source_weight || 5,
      alcaldia:      article.alcaldia,
      geo_confidence: article.geoConfidence || article.geo_confidence || 0,
      category:      article.category || 'general',
      pub_date:      article.pubDate || article.pub_date || new Date().toISOString(),
      scraped_at:    article.scrapedAt || article.scraped_at || new Date().toISOString(),
      status:        article.status || 'pending_rewrite',
    });
  },

  // GUARDAR NOTA YA REESCRITA POR LA IA y MOVERLA a 'published'.
  // Esto es lo que faltaba: antes la versión terminada se perdía.
  publishRewritten(article) {
    ...
    `).run({
      ...
      tags,
      social_tweet:     article.social_tweet || null,
      social_instagram: article.social_instagram || null,
      social_facebook:  article.social_facebook || null,
      category:         article.category || null,
      urgency:          article.urgency || null,
